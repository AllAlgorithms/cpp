...
void print(const vector<list<int> >& adjlist, int i, bool vis[])
...
void fillstack(const vector<list<int> >& adjlist, int i, stack<int>& order, bool vis[])
{
    vis[i]=true;
    list<int>::iterator it;
    for(it=adjlist[i].begin(); it!=adjlist[i].end(); it++)
...
int kosaraju(const vector<list<int> >& adjlist, int v)
{   
    int count=0;
    bool vis[v];
    int i;
