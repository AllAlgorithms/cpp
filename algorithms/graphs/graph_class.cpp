            ...
            for(auto i : adj[min_distance_vertex]) {
                if(!visited[i]) {
                    distance[i] = min(distance[i],
                                        min_distance + weight[min_distance_vertex][i]);
                }
            }
        }
        //do something....
        ...
        for(int i=0; i<vertices-1; i++) {
            for(auto it : edge_list) {
                if(distance[it.first] != INF)
                    distance[it.second]=
                        min(distance[it.second], 
                            distance[it.first]+weight[it.first][it.second]);
            }
        }
        //do something....
        for(auto i  : distance) cout<<i<<" ";
        ...
        for(int i=0; i<vertices; i++) {
            ...
            for(int j=0; j<vertices; j++) {
                ...
                for(int k=0; k<vertices; k++) {
                    weight_matrix[i][k] = min(weight_matrix[i][k],
                                                weight_matrix[i][j] + weight_matrix[j][k]);
                }            
            }
        }
